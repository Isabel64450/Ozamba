...
class AuthService {
  ...
  async registerUser(userData: RegisterUserData): Promise<RegisterUserResponse> {
    ...
    const userId = await this.authRepository.createUser(newUser);

    const jwtSecret = process.env.JWT_SECRET;
    if (!jwtSecret) throw new Error("JWT_SECRET n'est pas défini");
    ...
    const verificationUrl = `${clientFront}/verify-email/${verificationToken}`;
      ...
      await sendEmail.sendMail({
        ...
        html: `<div style="font-family: Arial, Helvetica, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 30px; color: #333333; background-color: #ffffff;">
          <div style="text-align: center; margin-bottom: 30px;">
            <h1 style="color: #2F4798; margin: 0; font-size: 28px;">Welcome to Ozamba</h1>
          </div>
          <p style="font-size: 16px; line-height: 1.6;">Hello <strong>${userName}</strong>,</p>
          <p style="font-size: 16px; line-height: 1.6;">Thank you for creating your Ozamba account. Please confirm your email address by clicking the button below.</p>
          <div style="text-align: center; margin: 35px 0;">
            <a href="${verificationUrl}" style="display: inline-block; padding: 14px 28px; background-color: #2F4798; color: #ffffff; text-decoration: none; border-radius: 8px; font-size: 16px; font-weight: bold;">
          ...
          <p style="font-size: 13px; word-break: break-all; color: #2F4798;">${verificationUrl}</p>
          <p style="margin-top: 30px; font-size: 14px; line-height: 1.6; color: #666666;">If you did not create this account, you can simply ignore this email.</p>
  ...
  async verifyEmail(token: string) {
    ...
    try {
      ...
      if (error instanceof jwt.JsonWebTokenError) throw new Error("INVALID_TOKEN");
      throw error;
    }

    if (!decoded.id) throw new Error("INVALID_TOKEN");
  ...
  async loginUser(loginData: LoginUserData): Promise<LoginUserResponse> {
    ...
    if (!passwordIsValid) throw new Error("INVALID_CREDENTIALS");
